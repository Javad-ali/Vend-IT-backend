...
export const upsertMachines = async (machines) => {
    ...
    const { error } = await supabase
        .from('machine')
        .upsert(machines.map(machine => mapRemoteMachine(machine)), { onConflict: 'u_id' });
    if (error)
        throw error;
};
...
export const upsertSlots = async (slots) => {
    ...
    for (let i = 0; i < slots.length; i += CHUNK_SIZE) {
        ...
        const payload = chunk.map(slot => {
            ...
            const maxQuantity = slot.max_quantity ?? slot.max_capacity ?? pickProp(slotRecord, 'maxCapacity');
...
export const getMachineSlots = async (machineUId) => {
    const { data, error } = await supabase
        .from('machine_slots')
        .select('slot_number, quantity, max_quantity, price, product:product_u_id (brand_name, description, product_image_url, unit_price)')
