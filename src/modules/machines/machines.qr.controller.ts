...
import { generateMachineQr, handleSilkronWebhook as processSilkronWebhook, handleTapWebhook as processTapWebhook } from './machines.qr.service.js';
import { getConfig } from '../../config/env.js';
const verifySilkronSignature = (req) => {
    const { silkronWebhookSecret } = getConfig();
    if (!silkronWebhookSecret)
        return true;
    const header = req.headers['x-webhook-token'] ??
        req.headers['x-silkron-token'] ??
