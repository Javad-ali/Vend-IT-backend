...
import { getPaymentByChargeId, getPaymentById, updatePaymentStatus } from '../payments/payments.repository.js';
...
const deriveStatusFromTapEvent = (payload) => {
    ...
    if (typeof type === 'string') {
        if (type.includes('refunded'))
            return 'REFUNDED';
        if (type.includes('captured'))
            return 'CAPTURED';
        if (type.includes('authorized'))
            return 'AUTHORIZED';
...
export const handleSilkronWebhook = async (headers, body) => {
    ...
    const paymentId = payload.payment_id ??
        payload.paymentId;
    const vendorParts = coerceStringArray(payload.vendor_part_numbers) ||
        coerceStringArray(payload.vendorPartNumbers);
