...
import { clearCart, deleteCartItem, findSlot, getCart, getCartItem, updateCartQuantity, upsertCartItem } from './cart.repository.js';
export const addToCart = async (userId, input) => {
    ...
    const unitPrice = slot.price ?? (Array.isArray(slot.product) && slot.product.length > 0 ? slot.product[0].unit_price : 0) ?? 0;
