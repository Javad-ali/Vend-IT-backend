...
const buildProductProperties = (metadata, remote) => {
    ...
    const addEntry = (key, value) => {
        ...
        if (!normalizedKey.length || seen.has(normalizedKey))
            return;
...
export const getProducts = async (machineUId, categoryId) => {
    ...
    let filtered = slots.filter(slot => {
        if (!slot.product)
            return false;
        if (isAllCategory || categoryId === 'ALL')
            return true;
...
export const searchProducts = async (machineUId, searchTerm) => {
    ...
    `)
        ...
        .or(`description.ilike.*${searchTerm}*,brand_name.ilike.*${searchTerm}*,vendor_part_no.ilike.*${searchTerm}*`, { referencedTable: 'product' });
    if (error)
        throw error;
...
export const getProductImage = async (productId) => {
    ...
    const categoryName = getRecordString(categoryRelation, 'category_name') ??
    ...
    const payload = {
        ...
        remark,
        vendor_part_no: product?.vendor_part_no ??
            (remote?.partNo ? String(remote.partNo) : null),
