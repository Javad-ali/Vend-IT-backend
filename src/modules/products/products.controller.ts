import { getCategories, getProductDetail, getProductImage, getProducts, searchProducts } from './products.service.js';
...
export const handleSearchProducts = async (req, res) => {
    ...
    if (!machineId || !searchTerm) {
        return res.status(400).json({ status: 400, message: 'machine_id and q query params are required' });
    }
...
export const handleLegacyProductImage = async (req, res) => {
    const productId = (req.body?.p_uid && String(req.body.p_uid)) ||
