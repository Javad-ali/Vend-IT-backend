...
export const ensureCategories = async (products) => {
    ...
    if (names.length) {
        const { data, error } = await supabase
            .from('category')
            .upsert(names.map(name => ({ category_name: name })), { onConflict: 'category_name' })
            .select();
        if (error)
            throw error;
    ...
    if (allCategoryId)
        categoryMap.set('All', allCategoryId);
...
export const upsertRemoteProducts = async (products) => {
    ...
    const payload = products.map(product => {
        const record = product;
        const vendorPart = product.vendor_part_no ??
        ...
        const detailImage = product.product_detail_image_url ?? product.image_url ?? recordImage ?? null;
        return {
    ...
    if (allCategoryId) {
        const { error: setAllCategoryError } = await supabase
            ...
            .is('category_id', null);
        if (setAllCategoryError)
            throw setAllCategoryError;
    }
};
