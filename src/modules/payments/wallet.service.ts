...
import { attachPaymentProducts, createPayment, ensureWallet, incrementWallet, decrementWallet, recordWalletTransaction, updateUserTapId, createLoyaltyEntry, incrementLoyaltyBalance, getWalletBalance, setPaymentEarnedPoints } from './payments.repository.js';
...
import { calculatePurchasePoints, calculateRedemption, applyRedemption } from './payments.service.js';
...
export const payWithWallet = async (userId, input) => {
    ...
    if (walletChargeAmount > 0) {
        await recordWalletTransaction({
            ...
            type: 'debit',
            amount: walletChargeAmount,
            metadata: redemption.pointsRedeemed > 0
