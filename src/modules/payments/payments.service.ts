...
import { attachPaymentProducts, createLoyaltyEntry, createPayment, getPaymentById, getUser, getWalletBalance, getLoyaltyBalance, incrementLoyaltyBalance, listPaymentHistory, listOrderHistory, listLoyaltyHistory, listWalletHistory, listPaymentProductQuantities, setDispensedQuantity, updateUserTapId, setPaymentEarnedPoints, setPaymentRedemption } from './payments.repository.js';
import { tapCreateCharge, tapCreateChargeWithToken, tapCreateCustomer, tapCreateGPayToken, tapCreateSavedCardToken } from './tap.client.js';
...
const extractHealthRating = (metadata) => {
    if (!metadata || typeof metadata !== 'object')
        return null;
    const value = metadata['health_rating'] ??
        metadata['healthRating'];
...
export const calculatePurchasePoints = async (items, amount) => {
    ...
    for (const item of list) {
        ...
        const multiplier = healthRating && healthRating >= 3
            ? HEALTHY_MULTIPLIER
            : healthRating === 1
...
export const makeCardPayment = async (userId, input) => {
    ...
    if (redemption.payableAmount > 0) {
        ...
        const token = await tapCreateSavedCardToken({ cardId: input.cardId, customerId: tapCustomerId });
    ...
    const points = await awardPurchasePoints(userId, payment.id, input.products, redemption.payableAmount);
    if (points > 0) {
...
export const saveIosPayment = async (userId, input) => {
    ...
    return ok({ payment, points, redeemedPoints: redemption.pointsRedeemed, redeemedAmount: redemption.redeemValue }, 'Payment saved');
};
...
export const getPaymentHistory = async (userId) => ok(await listPaymentHistory(userId), 'Payment history');
export const getWalletHistory = async (userId) => {
    ...
    const transactions = history.map(entry => {
        const payment = entry.payment && Array.isArray(entry.payment) ? entry.payment[0] : entry.payment;
        return {
            type: entry.type,
...
export const getLoyaltyHistory = async (userId) => ok(await listLoyaltyHistory(userId), 'Loyalty history');
