import { cardCreateSchema, cardPaymentSchema, dispenseSchema, gpayPaymentSchema, gpayTokenSchema, iosPaymentSchema, walletChargeSchema, walletPaymentSchema } from './payments.validators.js';
import { createCard, deleteCard, listCards } from './cards.service.js';
import { chargeWallet, payWithWallet } from './wallet.service.js';
import { createGPayToken, getLoyaltyHistory, getUserLoyaltyConversion, getOrderHistory, getPaymentHistory, getWalletHistory, makeCardPayment, makeGPayPayment, saveIosPayment, updateDispensedProducts } from './payments.service.js';
...
export const handleGPayToken = async (req, res) => {
    ...
    const response = await createGPayToken({ tokenData: payload.tokenData, paymentMethodType: payload.paymentMethodType });
    return res.json(response);
};
...
export const handlePaymentHistory = async (req, res) => res.json(await getPaymentHistory(req.user.id));
export const handleWalletHistory = async (req, res) => res.json(await getWalletHistory(req.user.id));
export const handleOrderHistory = async (req, res) => res.json(await getOrderHistory(req.user.id));
export const handleLoyaltyHistory = async (req, res) => res.json(await getLoyaltyHistory(req.user.id));
