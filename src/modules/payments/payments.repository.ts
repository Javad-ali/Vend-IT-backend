...
export const getUser = async (userId) => {
    const { data, error } = await supabase
        .from('users')
        .select('id, email, firstName:first_name, lastName:last_name, phoneNumber:phone_number, tapCustomerId:tap_customer_id')
        .eq('id', userId)
        .maybeSingle();
    if (error)
        throw error;
...
export const updateUserTapId = async (userId, tapCustomerId) => {
    const { error } = await supabase.from('users').update({ tap_customer_id: tapCustomerId }).eq('id', userId);
    if (error)
        throw error;
};
...
export const deleteCardByTapId = async (userId, tapCardId) => {
    const { error } = await supabase.from('cards').delete().eq('user_id', userId).eq('tap_card_id', tapCardId);
    if (error)
        throw error;
};
...
export const incrementWallet = async (userId, amount) => {
    const { data, error } = await supabase.rpc('wallet_increment', { p_user_id: userId, p_amount: amount });
    if (error)
        throw error;
    return data;
};
export const decrementWallet = async (userId, amount) => {
    const { data, error } = await supabase.rpc('wallet_decrement', { p_user_id: userId, p_amount: amount });
    if (error)
        throw error;
    return data;
};
...
export const attachPaymentProducts = async (payload) => {
    if (!payload.items.length)
        return;
    const { error } = await supabase.from('payment_products').insert(payload.items.map(item => ({
...
export const incrementLoyaltyBalance = async (userId, points) => {
    const { data, error } = await supabase.rpc('loyalty_increment', { p_user_id: userId, p_points: points });
    if (error)
        throw error;
...
export const listWalletHistory = async (userId) => {
    ...
    if (error)
        throw error;
    const paymentIds = (transactions ?? []).map(entry => entry.payment_id).filter((id) => Boolean(id));
