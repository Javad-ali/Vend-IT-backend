...
import { tapCreateCardToken, tapCreateCustomer, tapCreateSavedCardToken, tapListCards, tapDeleteCard } from './tap.client.js';
import { getUser, listCardsForUser, updateUserTapId, upsertCard, deleteCardByTapId } from './payments.repository.js';
...
export const deleteCard = async (userId, tapCardId) => {
    const user = await getUser(userId);
    if (!user?.tapCustomerId)
        throw new apiError(400, 'Customer ID not set');
    const deleted = await tapDeleteCard(user.tapCustomerId, tapCardId);
    if (!deleted)
        throw new apiError(400, 'Card not deleted');
