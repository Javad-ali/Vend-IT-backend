...
import { createReferralRecord, getReferralByInvitedUserId, getReferralStatsForInviter, markReferralRewarded, updateReferralRecord } from './referrals.repository.js';
...
const generateReferralCode = () => `${REFERRAL_CODE_PREFIX}${nanoid(REFERRAL_CODE_LENGTH).toUpperCase()}`;
...
export const processReferralReward = async (invitedUser, invitedPhone, context) => {
    if (!invitedPhone)
        return;
    const alreadyRewarded = invitedUser.referralRewardedAt ?? invitedUser.referral_rewarded_at ?? null;
    if (alreadyRewarded)
        return;
    ...
    if (!inviterPoints && !inviteePoints)
        return;
    const metadata = {
        ...(context.metadata ?? {}),
    ...
    else if (context.branchIdentity ||
