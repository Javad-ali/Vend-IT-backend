...
import { createNotification, getUserDeviceToken, listNotificationsByUser, markNotificationRead, clearNotifications } from './notifications.repository.js';
import { apiError, ok } from '../../utils/response.js';
const config = getConfig();
const { firebaseProjectId, firebaseClientEmail, firebasePrivateKey } = config;
const hasValidFcmCreds = Boolean(firebaseProjectId && firebaseClientEmail && firebasePrivateKey) &&
...
const createAccessToken = () => {
    ...
    }, firebasePrivateKey, { algorithm: 'RS256' });
};
...
export const sendNotification = async (input) => {
    ...
    if (!receiver?.device_token) {
        logger.warn({ receiverId: input.receiverId }, 'No device token found, stored notification only');
    ...
    try {
        ...
        await axios.post(`https://fcm.googleapis.com/v1/projects/${firebaseProjectId}/messages:send`, {
            message: {
                token: receiver.device_token,
                notification: input.title || input.body ? { title: input.title, body: input.body } : undefined,
                data: {
                    type: input.type ?? '',
