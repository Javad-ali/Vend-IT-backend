...
import { createCampaign, deleteCampaign, getCampaignById, getLatestCampaign, listCampaigns, recordCampaignView, updateCampaign } from './campaigns.repository.js';
...
export const fetchLatestCampaign = async (userId) => {
    ...
    try {
        ...
        logger.warn({ userId, campaignId: campaign.id }, 'Skipping campaign view record for deleted user');
    }
...
const uploadImage = async (file) => {
    ...
    if (error)
        throw new apiError(400, 'Failed to upload campaign image', error.message);
    return objectKey;
};
export const createCampaignWithMedia = async (payload) => {
    ...
    return ok({ ...campaign, image_url: buildImageUrl(campaign?.image_path ?? null) }, 'Campaign created');
};
export const updateCampaignWithMedia = async (id, payload) => {
    ...
    return ok({ ...updated, image_url: buildImageUrl(updated?.image_path ?? null) }, 'Campaign updated');
};
