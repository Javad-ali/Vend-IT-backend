...
import { createUser, getUserById, getUserByPhone, partialUpdateUser } from '../shared/repository.js';
...
const sendOtp = async (phone, otp) => {
    ...
    const shouldMock = smsDriver === 'log' || (process.env.NODE_ENV !== 'production' && smsDriver !== 'remote');
    if (shouldMock) {
        logger.info({ phone, otp }, 'Mock OTP dispatched');
        return true;
    }
    try {
        const { status } = await axios.post(process.env.SMS_API_URL ?? 'https://www.kwtsms.com/API/send/', new URLSearchParams({
...
const buildAuthResponse = (user, tokens, otp?) => {
    ...
    const userChatToken = user.userChatToken ??
        user.user_chat_token ??
        raw.user_chat_token ??
        null;
    const isNotification = Number(user.isNotification ?? user.is_notification ?? raw.is_notification ?? 0);
...
const updateUserCommonFields = (user, input, overrides) => {
    ...
    return {
        firstName: 'firstName' in input ? input.firstName ?? user?.firstName ?? null : user?.firstName ?? null,
        lastName: 'lastName' in input ? input.lastName ?? user?.lastName ?? null : user?.lastName ?? null,
        email: storedEmail,
        country: 'country' in input ? input.country ?? user?.country ?? null : user?.country ?? null,
...
export const loginUser = async (input) => {
    ...
    const updated = await partialUpdateUser(existing.id, {
        ...updateUserCommonFields(existing, { ...input, countryCode }, {
            otp,
