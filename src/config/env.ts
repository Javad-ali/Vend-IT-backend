...
export const getConfig = () => {
    ...
    const firebasePrivateKey = extractFirebasePrivateKey(process.env.FIREBASE_PRIVATE_KEY ?? serviceAccount?.private_key);
    const parsed = schema.safeParse({
        ...
        supabaseServiceRoleKey: process.env.SUPABASE_SERVICE_ROLE_KEY ?? (isTest ? 'stub-service-role-key' : undefined),
        ...
        tapApiBaseUrl: process.env.TAP_API_BASE_URL ??
            (isTest || isDev ? 'https://api.tap.company/v2' : undefined),
        ...
        tapCountryCode: sanitizeNumberEnv(process.env.TAP_COUNTRY_CODE) ?? (isTest || isDev ? 965 : undefined),
        ...
        remoteMachineBaseUrl: process.env.REMOTE_MACHINE_BASE_URL ?? (isTest ? 'https://example.com/rest' : undefined),
        remoteMachineApiKey: process.env.REMOTE_MACHINE_API_KEY ?? (isTest ? 'stub-remote-api-key' : undefined),
