 ...
 */
export const requireAdminToken = (req: Request, res: Response, next: NextFunction): void | Response => {
  try {
    const authHeader = req.headers.authorization;
    
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return res.status(401).json(
        errorResponse(401, 'No authorization token provided')
      );
    }
    ...
    return res.status(500).json(
      errorResponse(500, 'Authentication error')
    );
  }
};
 ...
 */
export { requireAdmin, requireAdminRole, attachAdmin, redirectIfAuthenticated, regenerateSession, destroySession } from './admin-auth.js';
